...
> **页面定位**：App 核心的新用户破冰与建档流程（Onboarding）。通过 8 步沉浸式对话式 UI，收集用户的身体目标、器材、水平、训练日、部位保护与生理信息，经 AI 生成态后展示策略愿景与今日任务，最终跳转自由训练。教练人设由上游流程（如 signup_flow）写入 session，本页只读展示。

---

### ⬅️ 顶部返回 `[plan_create_header_back]`

| 触发方式 | 🧩 模块层级 | 🔗 页面出口 |
| :--- | :--- | :--- |
| 点击左上角返回按钮 | Global | Step 1 时 `👉 intent_select`，其余步骤为上一 Step |
...
| 触发方式 | 🧩 模块层级 | 🔗 页面出口 |
| :--- | :--- | :--- |
| Step 7 点击 See Today's Tasks 后进入 | L1 核心 | `👉 free_mode` |

**📝 业务逻辑**：
* **内容展示**：展示教练问候卡片（教练信息来自上游 session，如 signup_flow 写入的 coach）、今日计划卡片（如 Ignite Sequence、时长、任务项）及数据预览（Calories / Intensity / Duration）。
* **动作/路由**：点击 "START WORKOUT" 跳转 `👉 free_mode`。
* **注册/数据**：真实业务中，点击 START WORKOUT 可在此或上一环节唤起注册/登录；注册成功后合并 Guest 与正式 User，将 Step 1–6 建档数据持久化落库。
...
**📝 业务逻辑**：
* **状态控制**：Step 1 未选目标、Step 2 未选器材、Step 3 未选水平时，对应 Continue 为 Disabled（透明度 30%）。
